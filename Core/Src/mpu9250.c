...
int initMPU9250()
{
    ...
    // read mpu9250 whoami
    uint8_t whoami;
    SPIReadSingle(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_WHOAMI | READ_FLAG, &whoami);
...
{
    // request read ak8963
    SPIWriteSingle(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_I2C_SLV0_ADDR, AK8963_I2C_ADDR | READ_FLAG);
    SPIWriteSingle(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_I2C_SLV0_REG, AK8963_HXL);
    SPIWriteSingle(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_I2C_SLV0_CTRL, 0x87);
    mpudebug("readMPU9250: ak8963 request");

    // read mpu9250
    uint8_t data[21];
    if (SPIRead(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_ACCEL_XOUT_H | READ_FLAG, 21, data) < 0)
    {
...
void calibrateMPU9250(float *dest1, float *dest2)
{
    ...
    SPIWriteSingle(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_FIFO_EN, 0x00);      // Disable FIFO
    SPIWriteSingle(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_PWR_MGMT_1, 0x00);   // Turn on internal clock source
    ...
    SPIWriteSingle(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_SMPLRT_DIV, 0x00);   // Set sample rate to 1 kHz
    SPIWriteSingle(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_GYRO_CONFIG, 0x00);  // Set gyro full-scale to 250 degrees per second, maximum sensitivity
    SPIWriteSingle(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_ACCEL_CONFIG, 0x00); // Set accelerometer full-scale to 2 g, maximum sensitivity
    ...
    uint8_t datah, datal;
    SPIWriteSingle(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_FIFO_EN, 0x00); // Disable gyro and accelerometer sensors for FIFO
    SPIReadSingle(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_FIFO_COUNTH | READ_FLAG, &datah);
    SPIReadSingle(HAMSTRONE_GLOBAL_SPI_PORT, (MPUREG_FIFO_COUNTH + 1) | READ_FLAG, &datal);
    ...
    for (ii = 0; ii < packet_count; ii++)
    {
        ...
        SPIRead(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_FIFO_R_W | READ_FLAG, 12, data); // read data for averaging
    ...
    SPIWriteSingle(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_XG_OFFS_USRH, data[0]);
    SPIWriteSingle(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_XG_OFFS_USRL, data[1]);
    SPIWriteSingle(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_YG_OFFS_USRH, data[2]);
    SPIWriteSingle(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_YG_OFFS_USRL, data[3]);
    SPIWriteSingle(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_ZG_OFFS_USRH, data[4]);
    SPIWriteSingle(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_ZG_OFFS_USRL, data[5]);
    ...
    SPIRead(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_XA_OFFSET_H | READ_FLAG, 2, data); // Read factory accelerometer trim values
    accel_bias_reg[0] = (int32_t)(((int16_t)data[0] << 8) | data[1]);
    SPIRead(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_YA_OFFSET_H | READ_FLAG, 2, data);
    accel_bias_reg[1] = (int32_t)(((int16_t)data[0] << 8) | data[1]);
    SPIRead(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_ZA_OFFSET_H | READ_FLAG, 2, data);
    ...
    SPIWriteSingle(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_XA_OFFSET_H, data[0]);
    SPIWriteSingle(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_XA_OFFSET_L, data[1]);
    SPIWriteSingle(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_YA_OFFSET_H, data[2]);
    SPIWriteSingle(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_YA_OFFSET_L, data[3]);
    SPIWriteSingle(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_ZA_OFFSET_H, data[4]);
    SPIWriteSingle(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_ZA_OFFSET_L, data[5]);
...
void calibrateAK8963()
{
    ...
    SPIWriteSingle(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_I2C_SLV0_ADDR, AK8963_I2C_ADDR | READ_FLAG); // Set the I2C slave    addres of AK8963 and set for read.
    SPIWriteSingle(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_I2C_SLV0_REG, AK8963_ASAX);                  // I2C slave 0 register address from where to begin data transfer
    SPIWriteSingle(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_I2C_SLV0_CTRL, 0x83);                        // Read 3 bytes from the magnetometer

    //WriteReg(MPUREG_I2C_SLV0_CTRL, 0x81);                     // Enable I2C and set bytes
    usleep(100000);
    //response[0]=WriteReg(MPUREG_EXT_SENS_DATA_01|READ_FLAG, 0x00); //Read I2C

    SPIWriteSingle(HAMSTRONE_GLOBAL_SPI_PORT, AK8963_CNTL1, 0x00); // set AK8963 to Power Down
    usleep(50000);                                                                   // long wait between AK8963 mode changes
    SPIWriteSingle(HAMSTRONE_GLOBAL_SPI_PORT, AK8963_CNTL1, 0x0F); // set AK8963 to FUSE ROM access
    usleep(50000);                                                                   // long wait between AK8963 mode changes

    SPIRead(HAMSTRONE_GLOBAL_SPI_PORT, MPUREG_EXT_SENS_DATA_00, 3, response);
    ...
    SPIWriteSingle(HAMSTRONE_GLOBAL_SPI_PORT, AK8963_CNTL1, MFS_16BITS << 4 | M_8HZ); // Set magnetometer data resolution and sample ODR
    usleep(50000);
}
